...
Mesh::Mesh(Vertex* vertices, unsigned int* indices, size_t vertexCount, size_t indexCount)
{
...
// File input object
	...
	this->vertexCount = (int)verts.size();
	this->indexCount = (int)indices.size();

	Mesh::CreateBuffer(&verts[0], &indices[0], verts.size(), indices.size());
...
{
	// Calculate tangents before creating and copying to buffers
	CalculateTangents(vertices, vertexCount, indices, indexCount);

	// Utilize the helper method from graphics to create static buffers for vertex and index
	...
	// Update the buffer views using the newly created buffers
...
void Mesh::CalculateTangents(Vertex* vertices, size_t vertexCount, unsigned int* indices, size_t indexCount)
{
	// Reset tangents
