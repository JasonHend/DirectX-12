...
void RayTracing::CreateRaytracingPipelineState(std::wstring raytracingShaderLibraryFile)
{
	...
	shaderConfigDesc.MaxPayloadSizeInBytes = sizeof(DirectX::XMFLOAT3)
		+ (2 * sizeof(unsigned int));	// Payload incudes float3 and an unsigned integer
	...
	std::vector<std::shared_ptr<GameEntity>> scene)
{
	...
	for (size_t i = 0; i < scene.size(); i++)
	{
		...
		instanceDesc.AccelerationStructure = scene[i]->GetMesh()->GetRayTracingData().BLAS->GetGPUVirtualAddress();
